...
function parseSharedIdentityAliases(value) {
  if (!value) return {};
  try {
    const parsed = JSON.parse(value);
    if (!parsed || Array.isArray(parsed) || typeof parsed !== 'object') return {};
    return Object.fromEntries(Object.entries(parsed)
      .filter(([alias, userId]) => typeof alias === 'string' && alias && typeof userId === 'string' && userId));
  } catch {
    return {};
  }
}
...
function parseIdleMinutes(value, fallback) {
  if (value === undefined || !/^\d+$/.test(String(value))) return fallback;
  const minutes = Number(value);
  return Number.isSafeInteger(minutes) ? minutes : fallback;
}
...
function loadConfig({ configPath = CONFIG_PATH } = {}) {
  const sharedIdentityAliases = parseSharedIdentityAliases(process.env.CAMOFOX_SHARED_IDENTITY_MAP);
  ...
  const headedIdleReleaseMinRaw = process.env.CAMOFOX_HEADED_IDLE_RELEASE_MIN;
  const hiddenTabIdleMinRaw = process.env.CAMOFOX_HIDDEN_TAB_IDLE_MIN;
  const hiddenSessionIdleMinRaw = process.env.CAMOFOX_HIDDEN_SESSION_IDLE_MIN;
  const headedIdleReleaseMin = parseIdleMinutes(headedIdleReleaseMinRaw, 30);
  const hiddenTabIdleMin = parseIdleMinutes(hiddenTabIdleMinRaw, 30);
  const hiddenSessionIdleMin = parseIdleMinutes(hiddenSessionIdleMinRaw, 120);
  ...
  return {
    ...
    headedIdleReleaseMin,
    hiddenTabIdleMin,
    hiddenSessionIdleMin,
    ...
    interactiveMode,
    // Opaque native Hermes userIds. The optional map lets a local launcher use
    // a human alias while resolving it to the exact same server-visible ID.
    sharedIdentityAliases,
    sharedIdentityNames: [...new Set([
      ...(Object.values(sharedIdentityAliases)),
      ...(process.env.CAMOFOX_SHARED_IDENTITIES || '').split(',').map(value => value.trim()).filter(Boolean),
    ])],
    sharedProfileDir: process.env.CAMOFOX_SHARED_PROFILE_DIR || join(os.homedir(), '.camofox', 'shared-profiles'),
    directIdentity,
    proxy: {
    ...
    serverEnv: {
      ...
      CAMOFOX_HEADED_IDLE_RELEASE_MIN: headedIdleReleaseMinRaw,
      CAMOFOX_HIDDEN_TAB_IDLE_MIN: hiddenTabIdleMinRaw,
      CAMOFOX_HIDDEN_SESSION_IDLE_MIN: hiddenSessionIdleMinRaw,
      CAMOFOX_INTERACTIVE: process.env.CAMOFOX_INTERACTIVE,
      CAMOFOX_SHARED_IDENTITY_MAP: process.env.CAMOFOX_SHARED_IDENTITY_MAP,
