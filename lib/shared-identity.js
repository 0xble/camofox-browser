...
export class SharedIdentityManager {
  ...
  async close(identity, { checkpoint = true } = {}) {
    ...
    let cookies;
    try {
      // A storage reset closes the profile-owning context specifically to
      // discard live session credentials. Do not read or write a clean
      // recovery checkpoint on that path: its existing dirty marker makes a
      // later reopen start without replaying session cookies.
      if (checkpoint) cookies = await context.cookies();
      await context.close?.();
