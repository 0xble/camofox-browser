...
export function browserErrorStatus(err) {
  if (err?.code === 'shared_page_unavailable' || err?.code === 'request_timeout') return 503;
...
export function browserErrorCode(err) {
  if (err?.code === 'shared_page_unavailable' || err?.code === 'request_timeout') return err.code;
...
export function browserErrorRecovery(err) {
  ...
  if (code === 'session_expired' || code === 'concurrency_timeout' || code === 'browser_launch_timeout' || code === 'shared_page_unavailable' || code === 'request_timeout') return 'retry';
  return null;
}
